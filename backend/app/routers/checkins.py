...
def list_checkins(
    ...
    total = query.count()
    checkins = (
        # Eager-load the relations checkin_to_dict touches (customer, room,
        # invoice) — otherwise each row triggers extra lazy-load queries (N+1).
        query.options(joinedload(Checkin.customer),
                      joinedload(Checkin.room),
                      joinedload(Checkin.invoice))
             .order_by(Checkin.checkin_datetime.desc())
