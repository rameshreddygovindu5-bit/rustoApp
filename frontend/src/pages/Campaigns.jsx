    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm p-12 text-center">
          <Megaphone size={40} className="mx-auto text-ink-300 mb-3"/>
          <p className="text-ink-500">No campaigns yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          {rows.map(c => {
            ...
            return (
              <div key={c.campaign_id} className="bg-white rounded-2xl shadow-card border border-ink-100 p-4">
                ...
                <p className="text-sm text-ink-700 bg-ink-50 rounded p-2.5 italic">"{c.message}"</p>
                {c.status === 'completed' && (
                ...
                <div className="mt-3 pt-3 border-t border-ink-100 flex gap-2">
                  <button onClick={() => setPreviewing(c)}
                          className="px-2.5 py-1 bg-ink-100 hover:bg-ink-200 text-ink-700 rounded text-xs flex items-center gap-1">
                    <Eye size={12}/> Preview audience
                  </button>
                  {c.status === 'draft' && (
                    <button onClick={() => handleSend(c)}
                            className="px-2.5 py-1 bg-gold hover:bg-gold/90 text-navy-dark rounded text-xs flex items-center gap-1 font-medium">
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <form onSubmit={submit} className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-3">
          ...
          <div>
            <label className="block text-xs font-semibold text-ink-600 uppercase tracking-wide mb-1">Message *</label>
            ...
            <p className="text-[11px] text-ink-400 mt-1">{f.message.length}/500 chars (multi-part SMS may cost more)</p>
          </div>
          <div>
            <label className="block text-xs font-semibold text-ink-600 uppercase tracking-wide mb-1">Audience</label>
          ...
          {f.audience_type === 'by_tier' && (
            <div>
              <label className="block text-xs font-semibold text-ink-600 uppercase tracking-wide mb-1">Tier</label>
          ...
          {f.audience_type === 'custom_list' && (
            <div>
              <label className="block text-xs font-semibold text-ink-600 uppercase tracking-wide mb-1">
                Phone numbers <span className="text-ink-400 normal-case">(one per line)</span>
              </label>
              <textarea value={(f.params.phones || []).join('\n')}
                        onChange={e => setF(s => ({ ...s, params: { phones: e.target.value.split('\n').map(p => p.trim()).filter(Boolean) }}))}
                        rows={4}
                        className="w-full px-3 py-2 border border-ink-300 rounded-lg font-mono text-sm"
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[80vh] flex flex-col">
        <div className="px-5 py-4 border-b border-ink-100 flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-ink-500 mt-0.5">{campaign.name}</p>
          </div>
          <button onClick={onClose} className="text-ink-400 hover:text-ink-600"><X size={20}/></button>
        </div>
        <div className="overflow-y-auto p-4 flex-1">
          {!data ? <div className="text-ink-400 text-center py-8">Loading…</div> :
            data.count === 0 ? (
              <div className="text-ink-500 text-center py-8 flex flex-col items-center gap-2">
                <Users size={32} className="text-ink-300"/> No recipients match this audience filter.
              </div>
            ) : (
              <>
                <div className="text-center mb-4">
                  <div className="text-4xl font-bold text-navy">{data.count}</div>
                  <div className="text-xs text-ink-500 uppercase tracking-wide">recipients</div>
                </div>
                <div className="text-xs text-ink-500 mb-1">First {data.sample.length}:</div>
                <div className="bg-ink-50 rounded p-2 text-xs font-mono space-y-0.5">
                  {data.sample.map((p, i) => <div key={i}>{p}</div>)}
                </div>
                {data.truncated && (
                  <p className="text-[11px] text-ink-400 mt-1">…and more not shown</p>
                )}
              </>
            )}
        </div>
