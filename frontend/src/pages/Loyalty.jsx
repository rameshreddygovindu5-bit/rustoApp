...
const TIER_META = {
  ...
  silver:   { label: 'Silver',   color: 'bg-ink-100 text-ink-700 border-ink-300' },
  gold:     { label: 'Gold',     color: 'bg-gold-50 text-gold-700 border-gold/40' },
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          <div className="bg-white rounded-2xl shadow-card border border-ink-100 p-4">
            <div className="text-[11px] uppercase tracking-wide text-ink-500">Total Accounts</div>
            <div className="text-2xl font-bold text-navy mt-1">{stats.total_accounts}</div>
          </div>
          {Object.entries(TIER_META).map(([tier, meta]) => (
            <div key={tier} className="bg-white rounded-2xl shadow-card border border-ink-100 p-4">
      ...
      <div className="bg-white rounded-2xl shadow-card border border-ink-100 overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12">
            <Award size={40} className="mx-auto text-ink-300 mb-3"/>
            <p className="text-ink-500">No loyalty accounts yet.</p>
            <p className="text-xs text-ink-400 mt-1">Accounts open automatically on first checkout.</p>
          </div>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {accounts.map(a => (
                <tr key={a.account_id} className="border-t border-ink-100 hover:bg-ink-50/50">
                  <td className="px-4 py-2.5 font-semibold text-navy">{a.customer_name || `#${a.customer_id}`}</td>
                  <td className="px-4 py-2.5 hidden md:table-cell text-ink-500 text-xs">{a.customer_phone || '—'}</td>
                  ...
                  <td className="px-4 py-2.5 text-right hidden sm:table-cell text-ink-500">{a.lifetime_points.toLocaleString('en-IN')}</td>
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[80vh] flex flex-col">
        ...
        <div className="overflow-y-auto p-3 flex-1">
          {loading ? (
            ...
            <div className="text-center text-ink-400 py-8">No transactions yet.</div>
          ) : (
            data.transactions.map(t => (
              <div key={t.txn_id} className="bg-ink-50 rounded p-2.5 mb-2 flex items-start gap-3">
                {t.points > 0
                  ? <TrendingUp size={16} className="text-green-500 mt-0.5"/>
                  : <TrendingDown size={16} className="text-red-500 mt-0.5"/>}
                <div className="flex-1">
                  <div className="flex justify-between items-baseline">
                    <span className="text-xs uppercase font-bold text-ink-500">{t.txn_type}</span>
                    <span className={`font-bold ${t.points > 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {t.points > 0 ? '+' : ''}{t.points} pts
                    </span>
                  </div>
                  {t.reason && <div className="text-xs text-ink-600 mt-0.5">{t.reason}</div>}
                  <div className="text-[10px] text-ink-400 mt-0.5">{new Date(t.created_at).toLocaleString()}</div>
                </div>
              </div>
            ))
          )}
