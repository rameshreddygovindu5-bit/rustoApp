  ...
  return (
    <div className="space-y-4 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          <h1 className="font-display text-xl sm:text-2xl font-bold text-navy">Check-ins</h1>
          <p className="text-xs text-ink-400 mt-0.5">{total} records found</p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          <button onClick={fetchCheckins} className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-3 py-2 text-sm border border-ink-200 rounded-xl text-ink-600 hover:bg-ink-50">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-ink-100 p-3 sm:p-4">
        <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center justify-between">
          <div className="flex gap-1 bg-ink-100 rounded-xl p-1 overflow-x-auto no-scrollbar">
            {[
              ...
              <button key={t.key} onClick={() => { setTab(t.key); setPage(1); }}
                className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${tab === t.key ? "bg-white text-navy shadow-sm font-semibold" : "text-ink-500 hover:text-navy"}`}>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-ink-100 overflow-hidden">
        ...
        <div className="md:hidden divide-y divide-ink-100">
          {loading ? Array(3).fill(0).map((_, i) => <div key={i} className="p-4 animate-pulse space-y-3"><div className="h-4 bg-ink-100 w-1/3" /><div className="h-3 bg-ink-50 w-1/2" /></div>)
            : checkins.length === 0 ? <div className="p-8 text-center text-ink-400"><AlertCircle size={32} className="mx-auto mb-2 opacity-40" /><p className="text-sm">No check-ins found</p></div>
            : checkins.map((c, i) => {
                ...
                <div key={c.checkin_id}
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-bold text-navy line-clamp-1">{c.customer?.first_name} {c.customer?.last_name}</p>
                        <p className="text-xs text-ink-500">{c.customer?.phone}</p>
                      </div>
                    </div>
                    <span className="bg-navy/10 text-navy text-xs font-bold px-2 py-1 rounded-lg">Rm {c.room_number}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-[11px]">
                    <div><p className="text-ink-400">In</p><p className="font-medium">{formatDateTime(c.checkin_datetime)}</p></div>
                    <div><p className="text-ink-400">Tariff</p><p className="font-medium">₹{c.tariff_per_night?.toLocaleString("en-IN")}</p></div>
                    {c.expected_checkout && (
                      <div className="col-span-2"><p className="text-ink-400">Expected Out</p><p className="font-medium">{formatDateTime(c.expected_checkout)}</p></div>
                    )}
                  </div>
                  <div className="flex items-center justify-between pt-1">
                    <StatusBadge status={c.status} isOverdue={isOverdue} />
                    <div className="flex gap-2">
                      <button onClick={() => setSelectedCheckin(c)} className="p-2 text-navy border border-ink-100 rounded-lg"><Eye size={14} /></button>
                      {c.status === "active" && (
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-ink-50 border-b border-ink-100">
                <th className="text-left text-xs font-semibold text-ink-500 uppercase tracking-wider px-6 py-3">Guest</th>
                <th className="text-left text-xs font-semibold text-ink-500 uppercase tracking-wider px-6 py-3">Room</th>
                <th className="text-left text-xs font-semibold text-ink-500 uppercase tracking-wider px-6 py-3">Dates</th>
            ...
            <tbody className="divide-y divide-ink-100">
              {checkins.map(c => (
                <tr key={c.checkin_id} className="hover:bg-ink-50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 bg-navy/10 rounded-full flex items-center justify-center text-navy text-xs font-bold">{c.customer?.first_name?.[0]}{c.customer?.last_name?.[0]}</div>
                      <div><p className="text-sm font-medium">{c.customer?.first_name} {c.customer?.last_name}</p><p className="text-xs text-ink-400">{c.customer?.phone}</p></div>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      <button onClick={() => setSelectedCheckin(c)} className="p-1.5 text-ink-400 hover:text-navy hover:bg-navy/10 rounded-lg"><Eye size={14} /></button>
                      {c.status === "active" && (
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-ink-100 flex items-center justify-between">
            <p className="text-[10px] sm:text-xs text-ink-500">Page {page} of {totalPages}</p>
            <div className="flex gap-1 sm:gap-2">
              <button disabled={page === 1} onClick={() => setPage(p => p - 1)} className="px-2 sm:px-3 py-1.5 text-xs border border-ink-200 rounded-lg disabled:opacity-40 hover:bg-ink-50">Prev</button>
              <button disabled={page === totalPages} onClick={() => setPage(p => p + 1)} className="px-2 sm:px-3 py-1.5 text-xs border border-ink-200 rounded-lg disabled:opacity-40 hover:bg-ink-50">Next</button>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="text-center mb-5">
              ...
              <p className="text-sm text-ink-500 mt-1">Room {checkoutTarget.room_number}: {checkoutTarget.customer?.first_name} {checkoutTarget.customer?.last_name}</p>
            </div>
            <div className="space-y-3 mb-5">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] font-bold text-ink-400 uppercase">Extra Charges</label>
                  <input type="number" className="w-full mt-1 px-3 py-2 border border-ink-200 rounded-xl text-sm focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20"
                ...
                <div>
                  <label className="text-[10px] font-bold text-ink-400 uppercase">Manual Discount</label>
                  <input type="number" className="w-full mt-1 px-3 py-2 border border-ink-200 rounded-xl text-sm focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20"
              ...
              <div>
                ...
                <div className="flex gap-1 mt-1">
                  <input type="text" placeholder="Optional"
                         className="flex-1 px-3 py-2 border border-ink-200 rounded-lg text-sm font-mono uppercase focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20"
                  ...
                  <button type="button" onClick={handleValidatePromo}
                          ...
                          className="px-3 py-2 bg-ink-100 hover:bg-ink-200 text-ink-700 rounded-lg text-xs font-semibold disabled:opacity-40">
                    Apply
                  </button>
                </div>
                {promoPreview?.ok && (
              ...
              <div>
                <label className="text-[10px] font-bold text-ink-400 uppercase">Payment Mode</label>
...
function StatusBadge({ status, isOverdue }) {
  const map = { active: "bg-green-100 text-green-700", checked_out: "bg-ink-100 text-ink-600", cancelled: "bg-red-100 text-red-600" };
  return (
    <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${isOverdue ? "bg-red-100 text-red-700" : map[status] || "bg-ink-100 text-ink-600"}`}>
...
function Info({ label, value }) {
  return (<div><p className="text-[10px] text-ink-400 uppercase font-bold">{label}</p><p className="text-sm font-medium text-ink-800">{value || "—"}</p></div>);
}

function InvoiceCard({ invoice, onClose, onViewDetails, onDownload }) {
  return (
    <div className="bg-green-50 border border-green-200 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-3 animate-slide-down">
      <div className="flex items-center gap-3">
...
function InvoiceModal({ invoice, onClose, onDownload }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4 backdrop-blur-sm">
      <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto animate-pop-in">
        <div className="bg-navy p-6 text-white flex justify-between items-center">
          <div><h3 className="text-xl font-bold font-display text-gold">Invoice</h3><p className="text-xs opacity-60 mt-1">#{invoice.invoice_number}</p></div>
        ...
        <div className="p-6 space-y-3">
          ...
          <div className="flex gap-3 pt-3">
            <button onClick={onClose} className="flex-1 py-3 border rounded-2xl text-ink-600 font-bold">Done</button>
...
function InvoiceItem({ label, value, className = "" }) {
  return (<div><p className="text-[10px] font-bold text-ink-400 uppercase tracking-widest mb-1">{label}</p><p className={`text-base font-semibold text-navy ${className}`}>{value}</p></div>);
}
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-lg max-h-[92vh] flex flex-col">
        ...
        {/* Total banner */}
        <div className="bg-gradient-to-br from-navy/5 to-gold/10 px-5 py-3 border-b border-ink-100 flex justify-between items-baseline">
          <span className="text-xs text-ink-500 uppercase tracking-wide">Folio Total</span>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-3">
          {loading ? (
            <div className="text-ink-400 text-center py-6 text-sm">Loading…</div>
          ) : data.items.length === 0 ? (
            <div className="text-ink-400 text-center py-6 text-sm">No charges yet.</div>
          ) : (
            <div className="space-y-2">
              {data.items.map(it => (
                <div key={it.charge_id}
                     className={`flex items-start gap-3 p-2.5 rounded ${it.voided ? 'bg-ink-50 opacity-60 line-through' : 'bg-white border border-ink-100'}`}>
        ...
        {/* Add form */}
        <form onSubmit={handleAdd} className="px-5 py-4 border-t border-ink-100 bg-ink-50 space-y-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            ...
            <button type="submit" disabled={adding}
                    className="px-3 py-1.5 bg-gold hover:bg-gold/90 text-navy-dark rounded text-sm font-medium flex items-center justify-center gap-1 disabled:opacity-50">
              <Plus size={12}/> Add
            </button>
          </div>
        </form>
