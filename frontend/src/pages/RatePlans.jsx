    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow-sm p-12 text-center">
          ...
          <p className="text-gray-400 text-xs mt-1">Without plans, every room sells at its base tariff.</p>
        </div>
      ) : (
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {plans.map(p => (
                <tr key={p.plan_id} className="border-t border-gray-100 hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-2.5 text-center hidden sm:table-cell text-gray-500">{p.priority}</td>
  ...
  return (
    <div className="bg-gradient-to-br from-navy/5 to-gold/5 border border-gold/30 rounded-xl p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
        <div>
          <label className="block text-xs text-gray-600 uppercase tracking-wide mb-1">Base ₹</label>
          <input type="number" value={baseTariff} onChange={e => setBaseTariff(e.target.value)}
                 className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
        </div>
        <div>
          <label className="block text-xs text-gray-600 uppercase tracking-wide mb-1">Room type</label>
          <input type="text" value={roomType} onChange={e => setRoomType(e.target.value)}
                 placeholder="any"
                 className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
        </div>
        <div>
          <label className="block text-xs text-gray-600 uppercase tracking-wide mb-1">Date</label>
      ...
      {result && (
        <div className="mt-4 pt-4 border-t border-gold/20">
          <div className="flex justify-between items-baseline mb-2">
            <span className="text-sm text-gray-500">Effective tariff:</span>
            <span className="text-3xl font-bold text-navy">
              ₹{Number(result.effective_tariff).toLocaleString('en-IN', { maximumFractionDigits: 2 })}
            </span>
          </div>
          {result.applied_plans.length === 0 ? (
            <p className="text-xs text-gray-500">No plans matched — using base tariff.</p>
          ) : (
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <form onSubmit={submit} className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-3">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1">Days of Week</label>
                ...
                return (
                  <button key={bit} type="button" onClick={() => toggleDay(bit)}
                          className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-colors ${
                            on ? 'bg-gold border-gold text-white' : 'bg-white border-gray-300 text-gray-600 hover:bg-gray-50'
                          }`}>
                    {label}
                  </button>
                )
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1">
              Room Type <span className="text-gray-400 normal-case">(blank = all types)</span>
            </label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1">Adjustment</label>
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1">
                Value <span className="text-gray-400 normal-case">(negative = discount)</span>
              </label>
          ...
          <div className="grid grid-cols-2 gap-3 items-end">
            <div>
              <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1">
                Priority <span className="text-gray-400 normal-case">(lower runs first)</span>
              </label>
