...
import { User, BookOpen, LogOut, Phone, Mail, MapPin,
         Sparkles, ArrowRight } from "lucide-react-native";
...
/**
 * Account tab. Two views toggled by a header tab control:
 *   - Profile  (default) — name / phone / email + sign out
 *   - Bookings           — list with status pills + pay/cancel actions
 *
 * Unauthenticated users see a sign-in CTA instead.
 */
export default function Account() {
  ...
  if (!customer) return (
    <View style={styles.guestContainer}>
      ...
      <Text style={styles.guestTitle}>Sign in to Rusto</Text>
      <Text style={styles.guestDesc}>
        Save your bookings, get faster checkouts, and access your travel history.
      </Text>
      <View style={{ marginTop: 24, alignSelf: "stretch", gap: 8 }}>
        <Button title="Sign in" variant="gold" fullWidth
...
function TabBtn({ label, Icon, active, onPress }: { label: string; Icon: any; active: boolean; onPress: () => void }) {
  return (
      ...
      <Card>
        <View style={styles.profHeader}>
          ...
          <View>
            ...
            <Text style={styles.profMeta}>Member since {customer.last_login_at ? new Date(customer.last_login_at).getFullYear() : "—"}</Text>
          </View>
        </View>
        ...
        <Button title="Sign out" variant="outline" small
                 icon={<LogOut size={13} color={colors.navy}/>}
                 onPress={async () => {
                   await logout();
  ...
  return (
    <View style={styles.fieldRow}>
      ...
      <View style={{ flex: 1 }}>
        ...
        <Text style={styles.fieldValue}>{value || <Text style={{ color: colors.ink400, fontStyle: "italic" }}>Not set</Text>}</Text>
      </View>
    </View>
  );
}
...
function BookingsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert("Couldn't load bookings", errorMessage(e));
    } finally { setLoading(false); setRefreshing(false); }
  }, []);

  // Re-fetch every time the tab gains focus — picks up a fresh booking
  // the user just created without needing a manual pull-to-refresh.
  useFocusEffect(useCallback(() => { load(); }, [load]));

  const cancel = (b: Booking) => {
    Alert.alert(`Cancel booking?`, b.booking_ref,
      [
  ...
  if (bookings.length === 0) return (
    <ScrollView contentContainerStyle={{ padding: spacing.lg }}>
      <View style={styles.empty}>
        <BookOpen size={36} color={colors.ink300}/>
        <Text style={styles.emptyTitle}>No bookings yet</Text>
        <Text style={styles.emptyDesc}>Start exploring lodges across India.</Text>
        <View style={{ marginTop: 16 }}>
          <Button title="Browse lodges" variant="gold"
  ...
  return (
    <ScrollView contentContainerStyle={{ padding: spacing.lg, paddingBottom: spacing.huge }}
      ...
      {bookings.map(b => <BookingRow key={b.booking_id} b={b} onCancel={cancel}/>)}
    </ScrollView>
  );
}
...
function BookingRow({ b, onCancel }: { b: Booking; onCancel: (b: Booking) => void }) {
  ...
  return (
    <Card style={{ marginBottom: spacing.md }}>
      ...
      <Text style={styles.bookingLodge}>{b.lodge?.name || "Lodge"}</Text>
      ...
      <View style={styles.bookingGrid}>
        ...
        <Stat label="Room" value={`${b.room_type_label} × ${b.rooms_count}`}/>
        <Stat label="Total" value={inr(b.total_amount)} highlight/>
      </View>
      {(canPay || canCancel) && (
        <View style={{ flexDirection: "row", gap: 8, marginTop: spacing.md, paddingTop: spacing.md,
                       borderTopWidth: 1, borderTopColor: colors.ink100 }}>
          {canPay && (
            <Button title="Complete payment" variant="gold" small
                     icon={<ArrowRight size={11} color={colors.navyDark}/>}
                     onPress={() => router.push({ pathname: "/checkout/[bookingId]",
                                                    params: { bookingId: String(b.booking_id) } })}/>
          )}
          {canCancel && (
            <Button title="Cancel" variant="outline" small
                     onPress={() => onCancel(b)}/>
          )}
        </View>
      )}
...
function Stat({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <View style={{ minWidth: "45%", marginBottom: 4 }}>
      ...
      <Text style={[styles.statValue, highlight && { color: colors.goldDark, fontWeight: "700" }]}>{value}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  bookingRef: { fontSize: 11, color: colors.ink400, fontFamily: "Courier" },
